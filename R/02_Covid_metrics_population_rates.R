...
# SECTION: Compute metric rates per 10,000 population

# 3. Aggregate total metrics PER DAY
...
# Keep just two objects on my workspace "METRICS_daily","POPULATION_original"
...
write.csv(METRICS_daily_calc,here("Checks","METRICS_daily_calc.csv"), row.names = TRUE)
...
# There was one issue as Metrics and Population figures came from two different data sources (metrics,WDI population figures)
# country names where slightly different on each original database

# Data cleanse country names 
...
# I opted to use this approach below to recode Country names.
...
# 3.4.4 FINAL CHECK - this dataframe should return a tibble with 0 rowns in it.
